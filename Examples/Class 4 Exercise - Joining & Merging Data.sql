-- 1. Using a UNION ON the world.country table, combine all countries (repeating) with a life expectancy of > 80 years 
-- or a GNP > 1000000, with no repeats.   
SELECT a.`name` AS country #, GNP, LifeExpectancy
FROM world.country AS a
WHERE lifeexpectancy > 80

UNION 

SELECT a.`name` AS country   #, Gross_National_Product, LifeDuration
FROM world.country AS a  
WHERE GNP > 1000000;
...
-- 4.  In the “world” schema, obtain the name of all cities in North America that have a population over 1M, 
-- WHERE the primary language is English or Spanish (with repeats). 
